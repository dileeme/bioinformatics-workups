...
def local_alignment_affine_gap(s, t, gap_open=11, gap_extend=1):
    n, m = len(s), len(t)
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            gap_s[i][j] = max(match[i - 1][j] - gap_open, gap_s[i - 1][j] - gap_extend)
            gap_t[i][j] = max(match[i][j - 1] - gap_open, gap_t[i][j - 1] - gap_extend)
            diag = match[i - 1][j - 1] + BLOSUM62[s[i - 1]][t[j - 1]]
            match[i][j] = max(0, diag, gap_s[i][j], gap_t[i][j])
    ...
    aligned_s, aligned_t = [], []
    i, j = best_pos
    while i > 0 and j > 0 and match[i][j] > 0:
        if match[i][j] == match[i - 1][j - 1] + BLOSUM62[s[i - 1]][t[j - 1]]:
            aligned_s.append(s[i - 1])
            aligned_t.append(t[j - 1])
            i -= 1
            j -= 1
        elif match[i][j] == gap_s[i][j]:
            aligned_s.append(s[i - 1])
            aligned_t.append("-")
            i -= 1
        else:
...
score, aligned_s, aligned_t = local_alignment_affine_gap(s, t)
