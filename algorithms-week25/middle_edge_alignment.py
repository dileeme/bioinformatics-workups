...
def column_scores(s, t_prefix):
    n = len(s)
    prev = [-INDEL_PENALTY * i for i in range(n + 1)]
    for char in t_prefix:
        cur = [0] * (n + 1)
        cur[0] = prev[0] - INDEL_PENALTY
        for i in range(1, n + 1):
            cur[i] = max(prev[i - 1] + BLOSUM62[s[i - 1]][char], prev[i] - INDEL_PENALTY, cur[i - 1] - INDEL_PENALTY)
        prev = cur
    return prev
...
def middle_edge(s, t):
    ...
    forward_mid = column_scores(s, t[:mid])
    forward_next = column_scores(s, t[:mid + 1])
    backward_mid = column_scores(s[::-1], t[mid:][::-1])[::-1]
    backward_next = column_scores(s[::-1], t[mid + 1:][::-1])[::-1]

    best_score = None
    best_edge = None
    for i in range(n + 1):
        if i < n and mid < m:
            total = forward_mid[i] + BLOSUM62[s[i]][t[mid]] + backward_next[i + 1]
            if best_score is None or total > best_score:
                best_score, best_edge = total, ((i, mid), (i + 1, mid + 1))
        if mid < m:
            total = forward_mid[i] - INDEL_PENALTY + backward_next[i]
            if best_score is None or total > best_score:
                best_score, best_edge = total, ((i, mid), (i, mid + 1))
        if i < n:
            total = forward_mid[i] - INDEL_PENALTY + backward_mid[i + 1]
            if best_score is None or total > best_score:
                best_score, best_edge = total, ((i, mid), (i + 1, mid))
