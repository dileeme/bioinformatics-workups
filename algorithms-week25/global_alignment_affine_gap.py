...
def global_alignment_affine_gap(s, t, gap_open=11, gap_extend=1):
    n, m = len(s), len(t)
    match = [[NEG_INF] * (m + 1) for _ in range(n + 1)]
    gap_s = [[NEG_INF] * (m + 1) for _ in range(n + 1)]
    gap_t = [[NEG_INF] * (m + 1) for _ in range(n + 1)]
    match[0][0] = 0

    for i in range(1, n + 1):
        gap_s[i][0] = max(match[i - 1][0] - gap_open, gap_s[i - 1][0] - gap_extend)
    for j in range(1, m + 1):
        gap_t[0][j] = max(match[0][j - 1] - gap_open, gap_t[0][j - 1] - gap_extend)

    for i in range(1, n + 1):
        for j in range(1, m + 1):
            best_diag = max(match[i - 1][j - 1], gap_s[i - 1][j - 1], gap_t[i - 1][j - 1])
            match[i][j] = best_diag + BLOSUM62[s[i - 1]][t[j - 1]]
            gap_s[i][j] = max(match[i - 1][j] - gap_open, gap_s[i - 1][j] - gap_extend)
            gap_t[i][j] = max(match[i][j - 1] - gap_open, gap_t[i][j - 1] - gap_extend)

    return max(match[n][m], gap_s[n][m], gap_t[n][m])
